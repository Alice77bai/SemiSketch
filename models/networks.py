...
class Gate_IFTE(nn.Module):
    def __init__(self, dim, bias=False):
        super(Gate_IFTE, self).__init__()
        ...
        self.structure_gate = nn.Sequential(nn.Conv2d(in_channels=6, out_channels=3, kernel_size=3, stride=1,padding=1),nn.Sigmoid())
        self.texture_gate = nn.Sequential(nn.Conv2d(in_channels=6, out_channels=3, kernel_size=3, stride=1,padding=1),nn.Sigmoid())
    ...
    def forward(self, x):
        x_dec, x_skip = x
        #######生成门控##########
        energy = torch.cat((x_dec, x_skip), dim=1)
        gate_structure_to_texture = self.structure_gate(energy)
        gate_texture_to_structure = self.texture_gate(energy)

        dec = x_dec + self.texture_gamma * (gate_structure_to_texture * x_skip)
        skip = x_skip + self.structure_gamma * (gate_texture_to_structure * x_dec)
        con = torch.cat((dec, skip), dim=1)
        ...
        q, k = self.qk(self.qk_avg_pool(con)).chunk(2, dim=1)
        v = self.v(con)
        q = q.reshape(b, c, 1)
        k = k.reshape(b, c, 1)
        v = v.reshape(b, c, h * w)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        if no_antialias:
            ...
            model1 += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                       norm_layer(ngf * mult * 2),
                       nn.ReLU(True),
        ...
        if no_antialias:
            ...
            model2 += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                       norm_layer(ngf * mult * 2),
                       nn.ReLU(True),
                       Downsample(ngf * mult * 2)]
        ...
        if no_antialias_up:
            model5 += [nn.ConvTranspose2d(int(ngf * mult), int(ngf * mult / 2),
                                          kernel_size=3, stride=2,
                                          padding=1, output_padding=1,
                                          bias=use_bias),
                       norm_layer(int(ngf * mult / 2)),
        ...
        model6 += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        model6 += [nn.Tanh()]

        self.model = nn.Sequential(*model0,*model1,*model2,*model3,*model4,*model5,*model6)
        ...
        self.ifte = Gate_IFTE(3)

        self.get_g_nopadding = Get_gradient_nopadding()
        self.mix1 = CGAFusion(64, reduction=4)
        self.mix2 = CGAFusion(128, reduction=8)
        self.mix3 = CGAFusion(256, reduction=16)

        self.Fusion1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=True)
        self.outconv = nn.Sequential(nn.Conv2d(64, output_nc, kernel_size=1, stride=1, padding=0), nn.Tanh())
    ...
    def forward(self, input, layers=[], encode_only=False,mode='all',stop_layer=16):
        ...
        if len(layers) > 0:
            ...
            if mode=='encoder':
                ...
                ##################### 梯度下支路##################################
                grad_x = self.get_g_nopadding(input)
                out11 = self.model0(grad_x)  # 64
                out22 = self.model1(out11)  # 128
                out33 = self.model2(out22)  # 256
                out44 = self.model3(out33)  # 256
                out55 = self.model4(out44)  #128
                out66 = self.model5(out55)   # 64
                out77 = self.model6(out66)  # 3


                out1 = self.model0(input) #64
                m1 = self.mix1(out11, out1)
                out2 = self.model1(m1)  #128
                m2 = self.mix2(out22, out2)
                out3 = self.model2(m2)  #256
                m3 = self.mix3(out33, out3)
                out4 = self.model3(m3)  #256
                m4 = self.mix3(out44, out4)
                out5 = self.model4(m4)  #128
                m5 = self.mix2(out55, out5)
                out6 = self.model5(m5)  #64
                m6 = self.mix1(out66, out6)
                out7 = self.model6(m6)  #3

                fuse1 = self.ifte([out77, out7])
                fuse1 = self.Fusion1(fuse1)
                output = self.outconv(fuse1)

                return output, out77, grad_x #测试时删掉, out77, grad_x
...
class CGAFusion(nn.Module):
    def __init__(self, dim, reduction=8):
        ...
        self.coord_att = CoordAtt(dim, dim, reduction)
        self.conv = nn.Conv2d(dim, dim, 1, bias=True)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x, y):
        initial = x + y
        coord_att = self.coord_att(initial)
        result = initial + coord_att * x + (1 - coord_att) * y
