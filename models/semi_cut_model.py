...
class SEMICUTModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_NCE_s', type=float, default=0.1,
                            help='weight for StylePatchNCE loss: NCE(G(X^p), Y^p)')
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD_p = networks.define_D(netD_p_input_nc, opt.ndf, opt.netD, opt.n_layers_D, opt.normD,
                                            opt.init_type,
                                            opt.init_gain, opt.no_antialias, self.gpu_ids, opt)
    ...
    def forward(self):
        ...
        self.real_B_grad = self.get_g_nopadding(self.real_B)
        self.criterionL1 = torch.nn.L1Loss()

        # forward unpaired data
        self.fake,self.fake_B_grad, self.real_A_grad = self.netG(self.real)
        self.fake_B = self.fake[:self.real_A.size(0)]
        if self.opt.dce_idt:
            self.idt_B = self.fake[self.real_A.size(0):]

        # forward paired data
        self.fake_p,self.fake_B_p_grad, self.real_A_p_grad = self.netG(self.real_A_p)
        self.fake_B_p = self.fake_p[:self.real_A_p.size(0)]
        self.real_B_p_grad = self.get_g_nopadding(self.real_B_p)
    ...
    def compute_D_loss(self):
        ...
        # #梯度判别1###############################################
        # fake_AB_grad = self.fake_B_grad
        # pred_fake_grad = self.netD(fake_AB_grad.detach())
        # self.loss_D_fake_grad = self.criterionGAN(pred_fake_grad, False)
        #
        # # 梯度判别2##############################################
        # real_AB_grad = self.real_A_grad
        # pred_real_grad = self.netD(real_AB_grad)
        # self.loss_D_real_p_grad = self.criterionGAN(pred_real_grad, True)
    ...
    def compute_D_p_loss(self):
        ...
        fake_AB_p_grad = torch.cat((self.real_A_p_grad, self.fake_B_p_grad), 1)
        pred_fake_p_grad = self.netD_p(fake_AB_p_grad.detach())
        self.loss_D_fake_p_grad = self.criterionGAN(pred_fake_p_grad, False)


        real_AB_p_grad = torch.cat((self.real_A_p_grad, self.real_B_p_grad), 1)
        pred_real_grad = self.netD_p(real_AB_p_grad)
        self.loss_D_real_p_grad = self.criterionGAN(pred_real_grad, True)
        ...
        # combine loss and calculate gradients
        self.loss_D_p = (self.loss_D_p_fake + self.loss_D_p_real +self.loss_D_fake_p_grad +  self.loss_D_real_p_grad ) * 0.5
    ...
    def compute_G_loss(self):
        ...
        fake_AB_grad = torch.cat((self.real_A_p_grad, self.fake_B_p_grad), 1)
        pred_fake_grad = self.netD_p(fake_AB_grad)
        self.loss_G_GAN_grad = self.criterionGAN(pred_fake_grad, True)

        self.loss_G_L1 = self.criterionL1(self.fake_B_p, self.real_B_p) * self.opt.lambda_L1
        self.loss_G_L1_grad = self.criterionL1(self.fake_B_p_grad, self.real_B_p_grad) * self.opt.lambda_L1
        ...
        # 梯度StylePatchNCE loss
        self.loss_NCE_grad = self.calculate_NCE_loss(self.real_B_p_grad, self.fake_B_p_grad) * self.opt.lambda_NCE_s
        ...
        # add G_GAN_p loss
        self.loss_G += self.loss_G_GAN + self.loss_SRC + (self.loss_G_GAN_p + self.loss_NCE_s + self.loss_NCE_grad + self.loss_G_GAN_grad + self.loss_G_L1 + self.loss_G_L1_grad ) * 1.0   #lambda_pair尝试改为0.5
        return self.loss_G
